       ...
       public async void ShowAllCourses()
        {
            ...
            try
            {
                ...
                using (var context = new AppDbContext(options))
                {
                    ...
                    var model = ((ViewResult)result).Model as IEnumerable<Course>;
                    Assert.Equal("Big Ideas of Computer Science", model.FirstOrDefault().CourseTitle);
                    Assert.Equal("Introduction to Programming", model.LastOrDefault().CourseTitle);
                    sut.Dispose();
                }
            }
            finally
        ...
        public async void ReturnViewWhenInvalidModelState()
        {
            ...
            try
            {
                ...
                using (var context = new AppDbContext(options))
                {
                    ...
                    Assert.Equal(course.CourseDescription, model.CourseDescription);
                    Assert.Equal(course.CourseImageThumbnailUrl, model.CourseImageThumbnailUrl);
                    Assert.Equal(course.CourseNumber, model.CourseNumber);
                    Assert.Equal(course.MajorShortCode, model.MajorShortCode);
                    sut.Dispose();
                }
